//! 豆包 Volces API client and Rig integration
//!
//! # Example
//! ```
//! use rig::providers::volces;
//!
//! let client = volces::Client::new("YOUR_API_KEY");
//!
//! let doubao_lite = client.completion_model(volces::DOUBAO_LITE);
//! ```
...
// Main Volces Client
// ================================================================
const VOLCES_API_BASE_URL: &str = "https://ark.cn-beijing.volces.com/api/v3";
...
impl Client {
    /// Create a new Volces client builder.
    ///
    /// # Example
    /// ```
    /// use rig::providers::volces::{ClientBuilder, self};
    ///
    /// // Initialize the Volces client
    /// let volces = Client::builder("your-volces-api-key")
    ///    .build()
    /// ```
    ...
    /// Create a new Volces client. For more control, use the `builder` method.
    ///
    /// # Panics
    ...
    pub fn new(api_key: &str) -> Self {
        Self::builder(api_key)
            .build()
            .expect("Volces client should build")
    }
...
impl ProviderClient for Client {
    ...
    fn from_env() -> Self {
        let api_key = std::env::var("VOLCES_API_KEY").expect("VOLCES_API_KEY not set");
        Self::new(&api_key)
    }
...
impl CompletionClient for Client {
    ...
    /// Creates a Volces completion model with the given `model_name`.
...
/// The response shape from the Volces API
...
impl completion::CompletionModel for CompletionModel {
    ...
    > {
        ...
        tracing::debug!("Volces completion request: {request:?}");
        ...
        if response.status().is_success() {
            ...
            tracing::debug!(target: "rig", "Volces completion: {}", t);
...
// ================================================================
// Volces Completion API
// ================================================================

/// `doubao-lite` completion model
pub const DOUBAO_LITE: &str = "doubao-lite";
/// `doubao-pro` completion model
pub const DOUBAO_PRO: &str = "doubao-pro";
/// `doubao-pro-4k` completion model
...
mod tests {
    ...
    #[test]
    fn test_deserialize_volces_response() {
        let data = r#"{
            "choices":[{
                "finish_reason": "stop",
                "index": 0,
