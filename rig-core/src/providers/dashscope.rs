//! 阿里云 DashScope API client and Rig integration
//!
//! # Example
//! ```
//! use rig::providers::dashscope;
//!
//! let client = dashscope::Client::new("YOUR_API_KEY");
//!
//! let qwen_turbo = client.completion_model(dashscope::QWEN_TURBO);
//! ```
...
// ================================================================
// Main DashScope Client
// ================================================================
const DASHSCOPE_API_BASE_URL: &str = "https://dashscope.aliyuncs.com/api/v1";
...
impl Client {
    /// Create a new DashScope client builder.
    ///
    /// # Example
    /// ```
    /// use rig::providers::dashscope::{ClientBuilder, self};
    ///
    /// // Initialize the DashScope client
    /// let dashscope = Client::builder("your-dashscope-api-key")
    ///    .build()
    /// ```
    ...
    /// Create a new DashScope client. For more control, use the `builder` method.
    ///
    /// # Panics
    ...
    pub fn new(api_key: &str) -> Self {
        Self::builder(api_key)
            .build()
            .expect("DashScope client should build")
    }
...
impl CompletionClient for Client {
    ...
    /// Creates a DashScope completion model with the given `model_name`.
...
/// The response shape from the DashScope API
...
impl completion::CompletionModel for CompletionModel {
    ...
    > {
        ...
        tracing::debug!("DashScope completion request: {request:?}");
        ...
        if response.status().is_success() {
            ...
            tracing::debug!(target: "rig", "DashScope completion: {}", t);
...
// ================================================================
// DashScope Completion API
// ================================================================

/// `qwen-turbo` completion model
pub const QWEN_TURBO: &str = "qwen-turbo";
/// `qwen-plus` completion model
pub const QWEN_PLUS: &str = "qwen-plus";
/// `qwen-max` completion model
pub const QWEN_MAX: &str = "qwen-max";
/// `qwen-turbo-0624` completion model
pub const QWEN_TURBO_0624: &str = "qwen-turbo-0624";
...
mod tests {
    ...
    #[test]
    fn test_deserialize_dashscope_response() {
        let data = r#"{
            "output": {
                "choices": [{
                    "finish_reason": "stop",
        ...
        match result {
            Ok(response) => match &response.output.choices.first().unwrap().message {
                Message::Assistant { content, .. } => assert_eq!(content, "Hello, world!"),
                _ => panic!("Expected assistant message"),
            },
            Err(err) => {
