//! Vercel AI Gateway API client and Rig integration
//!
//! # Example
//! ```
//! use rig::providers::vercel_ai_gateway;
//!
//! let client = vercel_ai_gateway::Client::new("YOUR_GATEWAY_URL", "YOUR_API_KEY");
//!
//! let openai_gpt4 = client.completion_model("openai/gpt-4");
//! ```
...
impl Client {
    /// Create a new Vercel AI Gateway client builder.
    ///
    /// # Example
    /// ```
    /// use rig::providers::vercel_ai_gateway::{ClientBuilder, self};
    ///
    /// // Initialize the Vercel AI Gateway client
    /// let gateway = Client::builder("your-api-key")
    ///    .base_url("https://your-gateway.vercel.app")
    ///    .build()
    /// ```
    ...
    /// Create a new Vercel AI Gateway client with custom base URL. For more control, use the `builder` method.
    ///
    /// # Panics
    ...
    pub fn new(base_url: &str, api_key: &str) -> Self {
        Self::builder(api_key)
            .base_url(base_url)
            .build()
            .expect("Vercel AI Gateway client should build")
    }
...
impl ProviderClient for Client {
    ...
    fn from_env() -> Self {
        let api_key =
            std::env::var("VERCEL_AI_GATEWAY_API_KEY").expect("VERCEL_AI_GATEWAY_API_KEY not set");
        let base_url = std::env::var("VERCEL_AI_GATEWAY_URL")
    ...
    fn from_val(input: crate::client::ProviderValue) -> Self {
        ...
        Self::new(VERCEL_AI_GATEWAY_BASE_URL, &api_key)
    }
}
...
impl CompletionClient for Client {
    ...
    /// Creates a Vercel AI Gateway completion model with the given `model_name`.
...
/// The response shape from the Vercel AI Gateway API
...
impl completion::CompletionModel for CompletionModel {
    ...
    > {
        ...
        tracing::debug!("Vercel AI Gateway completion request: {request:?}");
        ...
        if response.status().is_success() {
            ...
            tracing::debug!(target: "rig", "Vercel AI Gateway completion: {}", t);
...
// ================================================================
// Vercel AI Gateway Completion API
// ================================================================

// Common model identifiers for Vercel AI Gateway
/// OpenAI GPT-4 model through Vercel AI Gateway
pub const OPENAI_GPT4: &str = "openai/gpt-4";
/// OpenAI GPT-3.5-turbo model through Vercel AI Gateway
pub const OPENAI_GPT3_5_TURBO: &str = "openai/gpt-3.5-turbo";
/// Anthropic Claude model through Vercel AI Gateway
pub const ANTHROPIC_CLAUDE: &str = "anthropic/claude-3-opus-20240229";
/// Google Gemini model through Vercel AI Gateway
pub const GOOGLE_GEMINI: &str = "google/gemini-1.5-pro-latest";
...
mod tests {
    ...
    #[test]
    fn test_deserialize_vercel_ai_gateway_response() {
        let data = r#"{
            "choices":[{
                "finish_reason": "stop",
                "index": 0,
